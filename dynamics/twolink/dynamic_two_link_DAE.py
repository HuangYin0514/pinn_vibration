...
class Phi_Net(nn.Module):
    ...
    def get_phi_q(self, q):
        ...
        #     [
        #         torch.stack(
        #             [e1, e0, self.l[0] * torch.sin(q[:, 2]) / 2, e0, e0, e0], dim=-1
        #         ),
        #         torch.stack(
        #             [e0, e1, -self.l[0] * torch.cos(q[:, 2]) / 2, e0, e0, e0], dim=-1
        #         ),
        #         torch.stack(
        #             [
        #                 e0,
        ...
        phi_q = torch.stack(
            [
                ...
                torch.stack(
                    [e0, e1, -self.l[0] * torch.cos(q[:, 2]) / 2, e0, e0, e0],
                    dim=-1,
                ),
                torch.stack(
                    [
...
class F_Net(nn.Module):
    ...
    def forward(self, t, coords):
        ...
        t = t.reshape(-1, 1)
        F = torch.tensor(
            [0, -self.m[0] * self.g, 0, 0, -self.m[1] * self.g, 0],
            device=self.device,
            dtype=self.dtype,
        )
...
#
#######################################################################
class DynamicTwoLinkDAE(nn.Module):
    def __init__(self, config, logger):
        super(DynamicTwoLinkDAE, self).__init__()
        self.device = config.device
        self.dtype = config.dtype
        self.config = config
        ...
        self.calculator = DynamicTwoLinkCalculator(config=config, logger=logger)
...
#
#######################################################################
class DynamicTwoLinkCalculator:
    def __init__(self, config, logger):
        self.config = config
        self.logger = logger
        self.m_net = M_Net(config, logger)
